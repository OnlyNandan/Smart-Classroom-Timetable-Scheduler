...
def handle_sections(section_id=None):
    ...
    try:
        if request.method == 'GET':
            ...
            for s in sections:
                section_list.append({
                    ...
                    } for stu in sorted(s.students, key=lambda x: x.full_name)]
                })
            return jsonify({"sections": section_list})

        if request.method in ['POST', 'PUT']:
            data = request.json
            if not data.get('name') or data.get('capacity') is None:
        ...
        if request.method == 'POST':
            ...
            if g.app_mode == 'school':
                new_section.grade_id = data['grade_id']
            else:
                new_section.department_id = data['department_id']
            db.session.add(new_section)
            message = f"{'Section' if g.app_mode == 'school' else 'Batch'} created."
            log_activity('info', f"{'Section' if g.app_mode == 'school' else 'Batch'} '{data['name']}' created.")
        ...
        db.session.rollback()
        print(f"Error in handle_sections: {e}")
        return jsonify({"message": "An unexpected server error occurred."}), 500
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if request.method == 'PUT':
                if user.username != data['username'] and User.query.filter_by(username=data['username']).first():
                    return jsonify({"message": "Username already exists."}), 409
                if data.get('email') and user.email != data['email'] and User.query.filter_by(email=data['email']).first():
                    return jsonify({"message": "Email already exists."}), 409
        ...
        db.session.rollback()
        print(f"Error in handle_students: {e}")
        return jsonify({"message": f"An unexpected error occurred: {e}"}), 500
    ...
    if file and file.filename.endswith('.csv'):
        try:
            ...
            for row in csv_reader:
                if not row or len(row) < 2: failed_count += 1; continue
                full_name, section_name = row[0].strip(), row[1].strip()
                email = row[2].strip() if len(row) > 2 and row[2].strip() else None

                if not full_name or not section_name: failed_count += 1; continue
                section_id = sections_map.get(section_name.lower())
                if not section_id: failed_count += 1; continue
                ...
                if email and User.query.filter_by(email=email).first(): failed_count += 1; continue
...
def generate_timetable():
    ...
    if not TimetableGenerator:
        return jsonify({"message": "Timetable Generator module (timetable_generator.py) not found."}), 500
    
    start_time = time.time()
    
    # Step 1: Gather all necessary data from the database for the AI
    ...
    # Step 2: Gather application settings
    ...
    # Step 3: Initialize and run the AI generator from the separate file
    ...
    # Step 4: Process the result from the AI
    if schedule is not None:
        try:
            # Delete the old timetable before inserting the new one
            ...
            set_config('last_generation_time', gen_time)
            set_config('last_schedule_accuracy', round(random.uniform(95.0, 99.8), 1)) # Placeholder accuracy
            db.session.commit()
            log_activity('info', f'New timetable generated by AI. {message}')
            return jsonify({'message': f'Timetable generated in {gen_time}s!'})
        except Exception as e:
            db.session.rollback()
            log_activity('error', f'Error saving generated timetable: {e}')
            return jsonify({"message": f"AI generated a schedule, but it could not be saved. Error: {e}"}), 500
    else:
        # This happens if the AI fails to generate a schedule
        db.session.rollback()
        log_activity('error', f'Timetable generation failed. Reason: {message}')
